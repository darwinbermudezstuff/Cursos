...
namespace Bussines
{
    ...
    public class CategoryBussines
    {
        ...
        public class Select
        {
                ...
                try
                {
                    ...
                    if (!GetCategory.Item1.Error)
                    {
                        response.Category = new Category()
                        {
                            ...
                            CellarAreaID = (int)GetCategory.Item2.idCellarArea,
            ...
            /// <summary>
            /// Return Category List To Specific Father Category
            /// </summary>
            /// <param name="request">Father Category ID</param>
            /// <returns>Category List</returns>
            public static GetCategoryResponse GetCategoryToFather(GetCategoryRequest request)
            {
                GetCategoryResponse response = new GetCategoryResponse();
                response.CategoryList = new List<Category>();
                response.Error = new Handler.ErrorObject();                
                ...
                try
                {
                    var GetCategory = CategoryData.Select.GetCategoryToFather(request.CategoryID);
                    if (!GetCategory.Item1.Error)
                    {
                        foreach (var item in GetCategory.Item2)
                        {
                            ...
                            if (item2.CategoryID > 0)
                            {
                                if (item.id == item2.CategoryID)
                                {
                                    CategoryList.Add(new Category()
                                    {
                                        id = item2.id,
                                        name = item2.name,
                                        CellarAreaID = item2.CellarAreaID,
            ...
            public static GetCategoryResponse Category(GetCategoryResponse request)
            {
                ...
                try
                {
                    tblCategory Category = new tblCategory()
                    {
                        ...
                        idCellarArea = request.Category.CellarAreaID,
        ...
        public class Update
        {
            ...
            public static GetCategoryResponse Category(GetCategoryResponse request)
            {
                ...
                try
                {
                    ...
                    var result = CategoryData.Update.CategoryAndAllChildCellarArea(Category);
                        if (result.Item1.Error)
                        {
                            response.Error.InfoError(result.Item1);
                        }
